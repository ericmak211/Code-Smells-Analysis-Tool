...
# Calculates the coding time ratio of the commits on the file
def check_coding_frequency(repo_path, file_path, num_commits):
    try:
        ...
        if len(commits) < 2:
            print("Not enough commits to analyze coding frequency.")
            return None
        
        coding_time_ratios = []
        
        # Calculate coding time ratios between consecutive commits
        ...
        if not coding_time_ratios:
            ...
            return 0.0  # Return 0.0 if no refactoring is detected
        
        # Calculate average coding time ratio
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            if line.startswith('cloned_repo'):
                parts = line.split(': ', 1)
                if len(parts) == 2:
                    path = parts[0].split(' ', 1)[0]  # Extract path, message, and issue type
                    message = parts[1]
...
def main(repo_url, num_commits):
    ...
    # Analyze coding frequency for each file
    for file_path in all_files:
        print("*****************************************************************************************************")
        print(f"Selected file for analysis: {file_path}")
        print("*****************************************************************************************************")

        # Check coding frequency
        file_path_in_repo = os.path.relpath(file_path, clone_dir)
        coding_ratio = check_coding_frequency(clone_dir, file_path_in_repo, num_commits)

        # Provide insights based on coding frequency for the current file
        if coding_ratio is None:
            print("\nNot enough data for coding frequency analysis.\n")
        elif coding_ratio == 0:
            print("\nNo significant refactoring activity detected in the specified commits.\n")
        elif coding_ratio <= 16:
        ...
        print("*****************************************************************************************************\n")
        print("Analyzing the code for issues...\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python Refactoring_Analysis_Tool.py <repository_url> <number_of_commits>")
    else:
        repo_url = sys.argv[1]
